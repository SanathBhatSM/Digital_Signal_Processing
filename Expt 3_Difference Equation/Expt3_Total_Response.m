%Matlab Program to find Total Response 
%From the given Difference Equation

%Total response of the system is the output of the system 
%due to both input and initial conditions
...
%Consider the input as x(n)=(1/2)^n*u(n)
...
%To plot pole zero plot
b=[1,-3, 12] %These are the coefficients of x(n) in Eq(1)
a=[1,-7,12]%These are the coeff of y(n) in Eq(1)
figure %This will open a new figure window
zplane(b,a) %This is the command to show pole-zero plot
grid on
title('Pole-Zero Plot')

%Criteria to check stability

%Criterion 1 : If the systems produces an unbounded output 
%for a bounded input, then the system is unstable

%Criterion 2 : If there is at least one pole outside the unit circle
%in Z-plane, then the system is unstable
